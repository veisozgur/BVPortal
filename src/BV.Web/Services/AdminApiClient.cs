...
public sealed class AdminApiClient(IHttpClientFactory httpClientFactory, AuthSession session)
{
    ...
    public async Task<AdminDashboardSummaryModel?> GetDashboardAsync(CancellationToken cancellationToken = default) =>
        !session.IsAuthenticated ? null : await CreateClient().GetFromJsonAsync<AdminDashboardSummaryModel>("api/v1/admin/dashboard", cancellationToken);
    ...
    public async Task<IReadOnlyList<AdminDailyMetricModel>> GetDailyMetricsAsync(int days = 30, CancellationToken cancellationToken = default) =>
        !session.IsAuthenticated ? [] : await CreateClient().GetFromJsonAsync<List<AdminDailyMetricModel>>($"api/v1/admin/dashboard/daily-metrics?days={days}", cancellationToken) ?? [];
    ...
    public async Task<AdminQuoteDetailModel?> GetQuoteDetailAsync(Guid id, CancellationToken cancellationToken = default) =>
        !session.IsAuthenticated ? null : await CreateClient().GetFromJsonAsync<AdminQuoteDetailModel>($"api/v1/admin/quote-requests/{id}", cancellationToken);
    ...
    public async Task<ApiResult> ChangeQuoteStatusAsync(Guid id, int status, CancellationToken cancellationToken = default)
    {
        ...
        return response.IsSuccessStatusCode ? ApiResult.Ok("Teklif durumu güncellendi.") : ApiResult.Fail($"Teklif durumu güncellenemedi ({(int)response.StatusCode}).");
    }

    public async Task<IReadOnlyList<AdminOperationNoteModel>> ListOperationNotesAsync(Guid quoteRequestId, CancellationToken cancellationToken = default) =>
        await CreateClient().GetFromJsonAsync<List<AdminOperationNoteModel>>($"api/v1/admin/quote-requests/{quoteRequestId}/notes", cancellationToken) ?? [];

    public async Task<ApiResult> AddOperationNoteAsync(Guid quoteRequestId, string note, CancellationToken cancellationToken = default)
    {
        var response = await CreateClient().PostAsJsonAsync($"api/v1/admin/quote-requests/{quoteRequestId}/notes", new { note }, cancellationToken);
        return response.IsSuccessStatusCode ? ApiResult.Ok("Operasyon notu eklendi.") : ApiResult.Fail($"Operasyon notu eklenemedi ({(int)response.StatusCode}).");
    }

    public async Task<ApiResult> AnswerQuoteAsync(Guid quoteRequestId, AdminAnswerQuoteModel model, CancellationToken cancellationToken = default)
    {
        var response = await CreateClient().PostAsJsonAsync($"api/v1/admin/quote-requests/{quoteRequestId}/response", model, cancellationToken);
        if (response.IsSuccessStatusCode) return ApiResult.Ok("Fiyat teklifi kaydedildi ve müşteriye gönderildi.");
    ...
    public async Task<IReadOnlyList<AdminCustomerListItemModel>> ListCustomersAsync(string? search = null, CancellationToken cancellationToken = default)
    {
        if (!session.IsAuthenticated) return [];
        var path = "api/v1/admin/customers?page=1&pageSize=50";
        if (!string.IsNullOrWhiteSpace(search)) path += $"&search={Uri.EscapeDataString(search)}";
        return await CreateClient().GetFromJsonAsync<List<AdminCustomerListItemModel>>(path, cancellationToken) ?? [];
    }

    public async Task<AdminCustomerDetailModel?> GetCustomerAsync(Guid id, CancellationToken cancellationToken = default) =>
        !session.IsAuthenticated ? null : await CreateClient().GetFromJsonAsync<AdminCustomerDetailModel>($"api/v1/admin/customers/{id}", cancellationToken);
}
...
public sealed record AdminOperationNoteModel(Guid Id, Guid QuoteRequestId, Guid CreatedByUserId, string Note, DateTime CreatedAtUtc);
public sealed record AdminCustomerListItemModel(Guid Id, Guid UserId, string FullName, string PhoneNumber, string Email, string? OrganizationName, string? City, string? District, int QuoteCount, DateTime CreatedAtUtc);
public sealed record AdminCustomerDetailModel(Guid Id, Guid UserId, string FullName, string PhoneNumber, string Email, string? OrganizationName, string? TaxNumber, string? Address, string? City, string? District, DateTime CreatedAtUtc, DateTime? UpdatedAtUtc, IReadOnlyList<AdminCustomerQuoteModel> Quotes);
public sealed record AdminCustomerQuoteModel(Guid Id, string Title, int Type, int Status, DateTime CreatedAtUtc);
